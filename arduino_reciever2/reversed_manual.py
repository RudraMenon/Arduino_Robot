...
#        arduino_write_pin(PIN_10, False)
    if action == ACTION_BACKWARD:
        ...
        arduino_set_pwm(PIN_6, 0)
        arduino_set_pwm(PIN_9, motor_duty_cycle)
        arduino_set_pwm(PIN_10, 0)
    if action == ACTION_FORWARD:
        arduino_set_pwm(PIN_5, 0)
        arduino_set_pwm(PIN_6, motor_duty_cycle)
        arduino_set_pwm(PIN_9, 0)
        arduino_set_pwm(PIN_10, motor_duty_cycle)
    if action == ACTION_ROTATE_LEFT:
        arduino_set_pwm(PIN_5, 0)       #BACKWARD
        arduino_set_pwm(PIN_6, .9)      #FORWARD RIGHT
        arduino_set_pwm(PIN_9, 0)       #BACKWARD LEFT
        arduino_set_pwm(PIN_10, 0.25)     #FORWARD 
    if action == ACTION_ROTATE_RIGHT:
        arduino_set_pwm(PIN_5, 0)
        arduino_set_pwm(PIN_6, 0.9)
        arduino_set_pwm(PIN_9, 0)
        arduino_set_pwm(PIN_10, 0.5)
